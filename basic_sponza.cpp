...
int main( int argc, char** argv )
{
  ...
  frm.load_shader( lighting_shader, GL_VERTEX_SHADER, "../shaders/basic_sponza/lighting.vs" );
  frm.load_shader( lighting_shader, GL_FRAGMENT_SHADER, "../shaders/basic_sponza/lighting.ps" );

  GLuint gbuffer_shader = 0;
  frm.load_shader( gbuffer_shader, GL_VERTEX_SHADER, "../shaders/basic_sponza/gbuffer.vs" );
  frm.load_shader( gbuffer_shader, GL_FRAGMENT_SHADER, "../shaders/basic_sponza/gbuffer.ps" );
  ...
  frm.display( [&]
  {
    ...
    //glUniformMatrix4fv( gbuffer_mvp_mat_loc, 1, false, &light_viewproj[0][0] );
    glUniformMatrix4fv( gbuffer_normal_mat_loc, 1, false, &normal_mat[0][0] );

    for( auto& c : culled_objs )
    {
      if( s.meshes[c].local_bv->is_intersecting( &f ) )
        s.meshes[c].render( );
    }
    ...
    glUniformMatrix4fv( lighting_mvp_mat_loc, 1, false, &projection[0][0] );
